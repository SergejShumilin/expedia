...
public class GiftCertificateDaoImpl implements GiftCertificatesDao<GiftCertificate> {
    ...
    private final static String IS_EXISTS_BY_TAG = "SELECT EXISTS(SELECT certificates.name from certificates inner join tags on certificates.tag_id=tags.id where tags.name = ?)";
    ...
    @Override
    public List<GiftCertificate> findByTag(String tagName) {return jdbcTemplate.query(FIND_BY_TAG, new Object[]{tagName}, giftCertificateMapper); }

    @Override
    public List<GiftCertificate> findByDescription(String description) { return jdbcTemplate.query(FIND_BY_DESCRIPTION, new Object[]{"%"+description+"%"}, giftCertificateMapper); }
    ...
    public boolean isExistByTagName(String tagName){
        return jdbcTemplate.queryForObject(IS_EXISTS_BY_TAG, Boolean.class, tagName);
    }
    @Override
